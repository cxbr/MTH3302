...
def actual_daily_ema_test() -> dict:
    try:
        ...
        for day_number, day in enumerate(dates):
            # Two completed 15-minute bars per synthetic session are enough for
            # the production daily aggregation. The first 20 sessions establish
            # a valid EMA20 near 120. Session 21 closes at 100, below EMA20.
            for clock in ["09:30", "15:45"]:
                ts = pd.Timestamp(f"{day.date()} {clock}", tz="America/New_York")
                index.append(ts)
                if day_number < 20:
        ...
        entry_i = 40  # first bar of session 21
        signal = EntrySignal(
            "breakout",
            entry_i,
            entry_i,
        ...
        event = ReferenceEvent("TEST", 0, 0, 0, 0, entry_i - 1, 100.0)
        ...
        expected_exit = market.index[42]  # next session's first bar
        audit1.check(pd.Timestamp(trade["exit_time"]) == expected_exit, trade)
